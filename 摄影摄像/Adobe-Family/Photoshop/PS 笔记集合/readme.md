...
## ◆ 零散技巧汇总
### 图层蒙版不具备色彩，只有256个灰阶，白色表示完全不透明，黑色表示完全透明，不同的灰度值表示不同程度的透明。
### 选中画面的高光区域: ctr+alt+2   然后反选选择暗部shift+ ctr+ i
### 给图层填充颜色: 先用快捷键“/”  锁定透明图层.然后用填色快捷键如：alt  + delete 或 shift + backspace 等完成 
