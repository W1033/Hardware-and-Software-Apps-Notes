...
此时“媒体池”中和“源磁带”中仅显示“访谈媒体夹”中的片段，按照<span style="color:red;">片段名称和起始时间码排序</span>。

- 提示：按照片段名称和起始时间码排序(organized by clip name and grouped by the hour of the starting timecode.) 应该就是 **媒体夹(Bin)** 排序而不是**时间码(Timecode)**排序，判断依据来自下图。
...
首先点击下图中的（1） "慢搜入点" 按钮，检视器内部会分为两个窗口，左侧窗口显示的画面为 INTERVIEWS 文件夹中的第一个片段的第一帧，右侧
